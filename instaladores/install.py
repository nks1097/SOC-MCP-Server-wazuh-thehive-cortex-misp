#!/usr/bin/env python3
"""
SOC MCP Server v2.1.0 - Universal Installer
===========================================
...
def print_header():
    ...
    print(f"{Colors.CYAN}║           SOC MCP Server v2.1.0 Installer          ║{Colors.NC}")
...
def create_config_file() -> bool:
    ...
    if not env_file.exists():
        env_content = """# SOC MCP Server v2.1.0 Configuration
...
def create_claude_desktop_config() -> Tuple[str, str]:
    ...
    config = {
        "mcpServers": {
            "wazuh": {
                "command": python_path,
                "args": [str(current_dir / "bin" / "soc-mcp-server"), "--stdio"]
            }
        }
    }
...
def test_installation() -> bool:
    ...
    # Test server import
    test_cmd = [str(python_path), "-c", "import sys; sys.path.insert(0, 'src'); from soc_mcp_server.server import mcp; print('FastMCP server available')"]
...
def print_next_steps(config_path: str, config_json: str):
    ...
    print(f"• Test the server: {Colors.YELLOW}./bin/soc-mcp-server --stdio{Colors.NC}")
    print(f"• Run health check: {Colors.YELLOW}./bin/soc-mcp-server --health-check{Colors.NC}")
