#!/usr/bin/env python3
"""
SOC MCP Server - Complete MCP-Compliant Remote Server
...
from soc_mcp_server import __version__
...
from soc_mcp_server.auth import create_access_token
from soc_mcp_server.config import WazuhConfig, get_config
from soc_mcp_server.monitoring import ACTIVE_CONNECTIONS, setup_monitoring_middleware
from soc_mcp_server.resilience import GracefulShutdown
from soc_mcp_server.security import (
    RateLimiter,
    ToolValidationError,
    security_middleware,
...
from soc_mcp_server.session_store import SessionStore, create_session_store
...
async def lifespan(app: FastAPI):
    ...
    logger.info(f"SOC MCP Server v{__version__} starting up...")
    ...
    if cfg.is_oauth:
        try:
            from soc_mcp_server.oauth import create_oauth_router, init_oauth_manager
    ...
    if cfg.is_authless:
        ...
        if not os.getenv("MCP_API_KEY"):
            from soc_mcp_server.auth import auth_manager
    ...
    # === SHUTDOWN ===
    logger.info("🛑 SOC MCP Server initiating graceful shutdown...")
    ...
    try:
        ...
        from soc_mcp_server.security import connection_pool_manager
...
def create_error_response(
    ...
    from soc_mcp_server.monitoring import get_correlation_id
...
async def handle_initialize(params: Dict[str, Any], session: MCPSession) -> Dict[str, Any]:
    ...
    return {
        ...
        "instructions": "Connected to SOC MCP Server. Use available tools for security operations.",
    }
...
audit_logger = logging.getLogger("soc_mcp_server.audit")
...
async def handle_tools_call(params: Dict[str, Any], session: MCPSession) -> Dict[str, Any]:
    ...
    from soc_mcp_server.monitoring import record_tool_execution
...
async def process_mcp_request(request: MCPRequest, session: MCPSession) -> MCPResponse:
    ...
    try:
        ...
        from soc_mcp_server.monitoring import structured_logger
                ...
                return JSONResponse(
                    content={
                        ...
                        "result": {
                            ...
                            "serverInfo": {"name": "SOC MCP Server", "version": __version__},
...
async def metrics():
    ...
    from soc_mcp_server.monitoring import REGISTRY
